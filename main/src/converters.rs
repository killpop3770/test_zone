...
pub fn convert_md_to_html_parallel(
    files: Vec<PathBuf>,
    ...
    files
        ...
        .map(|(index, file)| {
            ...
            thread::spawn(move || {
                ...
                let mut output_file =
                    fs::File::create(html_file_path).expect("Can not create html file!");
                output_file
                    .write_all(parsed_data.as_bytes())
                    .expect("Can not write data in file!");
